...
将**本身带有可识别 identity signal 的银行原始 description**转化为稳定、可聚合的 canonical pattern，供下游相关节点使用。

**核心思想：用 LLM 做理解，用字典做确定性。理解一次，缓存永久。**

这是整个系统学习能力的基础：Observations 按 pattern 聚合，Rules 按 pattern 匹配，置信度分类器按 pattern 查找历史，Coordinator 按 pattern 分组部分 PENDING 交易。Pattern 不稳定，一切下游匹配都会断裂。

**边界说明：** `standardize_description` 不是所有交易 `description` 的唯一来源。调用方必须先判断该交易是否存在可识别的 identity signal：

- 如果存在，且 signal 位于 `raw_description` 中，则调用本工具
- 如果 cheque 已提取到 `cheque_info.payee`，当前版本直接将 `payee` 写入 `description`，不经过本工具
- 如果不存在可识别 identity signal，则 `description = null`，不调用本工具
...
| 返回字段 | 下游用途 |
|----------|---------|
| `description` | canonical pattern。仅对调用本工具的交易返回非空值；调用方将其写入交易 `description` 字段 |
| `raw_description` | 银行原始描述回传，便于调用方保留审计上下文；不是本工具被调用的充分条件 |
| `cleaned_fragment` | 仅用于工具内部字典查找，不传递给下游节点 |
| `source` | 调用方应将其作为 `pattern_source` 写入交易数据 / Transaction Log，用于审计和可靠性判断 |
...
```
调用前提：交易已被调用方判定为“identity signal 位于 raw_description 中”
...
- fallback 结果可以继续进入主 Workflow，并将 `source` 作为 `pattern_source = "fallback"` 下传
- fallback 交易可以写入 Transaction Log，但**不写入 / 不更新 Observations**
- fallback 交易**不参与 Rules 升级判断**，避免用不稳定 pattern 污染学习层
...
| 调用方 | 调用时机 | 操作 |
|--------|---------|------|
| 数据预处理 Agent | 仅对 `raw_description` 本身带有可识别 identity signal 的交易调用 | 查找 + 缓存新条目（Layer 2 未命中时） |
