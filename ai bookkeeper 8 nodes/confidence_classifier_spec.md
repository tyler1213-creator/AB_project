...
本次 Node 3 改造先把 `policy_trace` 定义为分类器标准输出。`policy_trace` 是否进入 Transaction Log / Coordinator handoff / `report_draft`，留到下游接口对齐阶段统一决定。
...
→ 写入 Transaction Log：
     classified_by = ai_high_confidence
     ai_reasoning = 本次输出
→ `policy_trace` 作为 Node 3 输出的一部分保留
   （是否进入下游正式 contract，待后续对齐）
```
...
→ 不写 observations
→ 不写 Transaction Log
→ 将 classifier_output 保留给下游
→ `policy_trace` 作为 Node 3 内部结构化输出保留
   （是否进入 Coordinator 正式 handoff，待后续对齐）
```
...
- `policy_trace`：记录触发了哪些 policy packs，以及哪些 override evidence 使其最终仍可高置信度。当前先定义为 Node 3 的标准输出字段；下游是否持久化，后续再对齐
...
| 高置信度分类 | Transaction Log | 写入 `ai_reasoning` |
| PENDING | Coordinator Agent | 接收 Node 3 的 pending 结果 |
