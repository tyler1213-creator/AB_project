...
### Step 6: Description 标准化

```
→ 所有交易数据经过 Step 2-5 进入交易池后，统一执行 description 标准化
...
- `description` 字段：标准化后的 canonical pattern，供下游节点默认消费
- `pattern_source` 字段：记录 pattern 来源，取值 `dictionary_hit` / `llm_extraction` / `fallback`
