...
| 附带 profile 变更信息 | "对了这个客户上个月开始雇人了" | 调用 update_profile.py + 处理分类回复 |
...
```
→ 调用 update_profile.py
→ 如果 profile 变更影响当前 PENDING 交易的判断（如新增 account_relationship）
  → 受影响的交易调用 retrigger_workflow.py，start_node: "Node 1"
→ 如果不影响当前交易 → 仅更新 profile，继续处理其他回复
```

**可能触发 profile 更新的场景：**

- 银行账户变更（新开/关闭账户）→ 更新 bank_accounts / account_relationships
- 员工状态变更 → 更新 has_employees
- 老板个人消费习惯变更 → 更新 owner_uses_company_account
- 贷款变更 → 更新 loans
- HST 注册状态变更 → 更新 has_hst_registration
...
- ✅ 更新 profile.md（当 accountant 提供客户结构信息变更时）
...
| 组件 | 关系 |
| --- | --- |
| 置信度分类器（Node 3） | 提供所有 PENDING 交易及其分类器输出 |
| Profile.md | 提供客户背景信息 |
...
| Profile 更新后重处理 | Node 1（Profile 匹配） | 受影响的交易从 Node 1 重新走 workflow |
...
| 文件 | 操作 | 触发条件 |
| --- | --- | --- |
| Profile.md | 更新字段 | Accountant 在回复中提供客户结构信息变更 |
